...
test('SW-020 PostgreSQL relational foundation', { skip: !databaseUrl }, async (t) => {
  ...
  try {
    await t.test('all seventeen ordered migrations are recorded', async () => {
      ...
      assert.deepEqual(result.rows.map((row) => row.version), [
        ...
        '016_legacy_readonly_importer.sql','017_shadow_migration_parity.sql'
      ]);
    });
    ...
    await t.test('migration evidence and shadow parity tables are tenant-bound and fingerprint/mode idempotent', async () => {
      await withRollback(pool, async (client) => {
        const ids = await seedTwoSchoolFixture(client);
        const fingerprint='a'.repeat(64);
        const inserted=await client.query<{id:string}>(`INSERT INTO migration_import_runs (organization_id,school_id,source_snapshot_fingerprint,source_alias,source_schema_version,source_exported_at,mode,status,finished_at) VALUES ($1,$2,$3,'synthetic-v18','2026-09-05-session-a',TIMESTAMPTZ '2026-09-08 20:00:00Z','IMPORT_SHADOW','PASS',now()) RETURNING id`,[ids.orgA,ids.schoolA,fingerprint]);
        const runId=inserted.rows[0]?.id; assert.ok(runId);
        await client.query(`INSERT INTO migration_shadow_records (organization_id,school_id,import_run_id,source_surface,source_ordinal,record_type,source_row_hash,state_json) VALUES ($1,$2,$3,'settings',0,'SETTING',$4,'{}'::jsonb)`,[ids.orgA,ids.schoolA,runId,'c'.repeat(64)]);
        await client.query(`INSERT INTO migration_shadow_parity_reports (organization_id,school_id,import_run_id,source_snapshot_fingerprint,overall_status) VALUES ($1,$2,$3,$4,'PASS')`,[ids.orgA,ids.schoolA,runId,fingerprint]);
        await expectPgConstraint(client,`INSERT INTO migration_import_runs (organization_id,school_id,source_snapshot_fingerprint,source_alias,source_schema_version,source_exported_at,mode,status,finished_at) VALUES ($1,$2,$3,'synthetic-v18','2026-09-05-session-a',now(),'IMPORT_SHADOW','PASS',now())`,[ids.orgA,ids.schoolA,fingerprint],['23505']);
        await expectPgConstraint(client,`INSERT INTO migration_shadow_records (organization_id,school_id,import_run_id,source_surface,source_ordinal,record_type,source_row_hash) VALUES ($1,$2,$3,'settings',1,'SETTING',$4)`,[ids.orgA,ids.schoolB,runId,'d'.repeat(64)],['23503']);
      });
    });

    await t.test('required hot-path indexes exist', async () => {
      const expectedIndexes = ['enrollments_current_section_student_idx','section_staff_current_lookup_idx','school_calendar_days_lookup_idx','school_policy_sets_effective_idx','audit_events_school_time_idx','transactional_outbox_pending_idx','transactional_outbox_delivery_ready_idx','transactional_outbox_processing_lease_idx','operations_job_runs_type_time_idx','operations_job_runs_school_time_idx','checkins_section_date_idx','checkins_student_date_idx','pass_requests_section_status_idx','queue_entries_fifo_idx','passes_section_status_idx','pass_events_resource_idx','pass_corrections_pass_time_idx','staff_actions_school_time_idx','classroom_connections_health_idx','section_external_links_due_idx','classroom_sync_runs_link_time_idx','classroom_roster_members_link_user_idx','integration_review_items_open_idx','migration_import_runs_school_time_idx','migration_import_runs_fingerprint_idx','legacy_id_mappings_schoolwide_idx','migration_reconciliation_findings_run_idx','migration_shadow_records_run_surface_idx','migration_shadow_records_school_type_idx','migration_shadow_records_identity_idx','migration_shadow_parity_school_time_idx'];
