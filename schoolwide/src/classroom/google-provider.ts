...
export class GoogleClassroomProvider implements ClassroomProvider {
  ...
  async completeAuthorization(input: {
    ...
    if (!pending || pending.kind !== 'PKCE') {
      throw new ClassroomProviderError('EXTERNAL_PROVIDER_ERROR', 'Classroom OAuth PKCE state is unavailable.', false);
    }
    ...
    const secret: GoogleClassroomSecret = {
      kind: 'TOKENS',
      accessToken: payload.access_token,
      ...(payload.refresh_token ? { refreshToken: payload.refresh_token } : {}),
      expiresAt: this.#now() + Math.max(1, payload.expires_in ?? 3600) * 1000,
    };
    await this.#vault.put(connectionRef, secret);
    await this.#vault.delete(input.pendingRef);
  ...
  async #accessToken(connectionRef: string): Promise<string> {
    ...
    if (!stored || stored.kind !== 'TOKENS') {
      throw new ClassroomProviderError('EXTERNAL_AUTH_REVOKED', 'Classroom authorization is unavailable.');
    }
    if (stored.expiresAt > this.#now() + 30_000) return stored.accessToken;
    if (!stored.refreshToken) {
      throw new ClassroomProviderError('EXTERNAL_AUTH_REVOKED', 'Classroom authorization has expired.');
    }
    const body = new URLSearchParams({
      client_id: this.#clientId,
    ...
    const payload = await response.json() as TokenResponse;
    if (!response.ok || !payload.access_token) {
      throw new ClassroomProviderError('EXTERNAL_AUTH_REVOKED', 'Classroom authorization was revoked.');
    }
  ...
  async #getJson<T>(connectionRef: string, url: URL): Promise<T> {
    ...
    if (response.status === 401 || response.status === 403) {
      throw new ClassroomProviderError('EXTERNAL_AUTH_REVOKED', 'Google Classroom authorization is no longer valid.');
    }
    if (!response.ok) {
      throw new ClassroomProviderError('SYNC_INCOMPLETE', 'Google Classroom returned an incomplete roster response.');
    }
  ...
  async listCourses(connectionRef: string, pageToken?: string): Promise<ClassroomPage<ClassroomCourse>> {
    ...
    return {
      items: (data.courses ?? []).map((course) => ({
        ...
        ...(course.ownerId ? { ownerId: course.ownerId } : {}),
  ...
  async listStudents(connectionRef: string, courseId: string, pageToken?: string): Promise<ClassroomPage<ClassroomPerson>> {
    ...
    return {
      items: (data.students ?? []).map((student) => ({
        id: student.userId,
        displayName: student.profile?.name?.fullName ?? `Classroom user ${student.userId}`,
        ...(student.profile?.emailAddress ? { email: student.profile.emailAddress } : {}),
      })),
      ...(data.nextPageToken ? { nextPageToken: data.nextPageToken } : {}),
    };
  }
  ...
  async listTeachers(connectionRef: string, courseId: string, pageToken?: string): Promise<ClassroomPage<ClassroomPerson>> {
    ...
    return {
      items: (data.teachers ?? []).map((teacher) => ({
        id: teacher.userId,
        displayName: teacher.profile?.name?.fullName ?? `Classroom user ${teacher.userId}`,
        ...(teacher.profile?.emailAddress ? { email: teacher.profile.emailAddress } : {}),
      })),
      ...(data.nextPageToken ? { nextPageToken: data.nextPageToken } : {}),
    };
  }
