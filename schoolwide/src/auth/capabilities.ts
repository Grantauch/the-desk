...
const teacherCapabilities: readonly Capability[] = [
  'teacher.section.read','teacher.section.live_state','teacher.section.attendance_backup','teacher.section.pass_start','teacher.section.pass_return','teacher.section.pass_review','teacher.section.pass_correct','teacher.section.student_access','teacher.section.policy_override','teacher.classroom.connect','teacher.classroom.import','teacher.classroom.sync'
];

const securityCapabilities: readonly Capability[] = [
  'security.live.read','security.student.lookup_live','security.pass.mark_located','security.pass.request_return','security.destination.read'
];
...
export function capabilitiesForRole(role: StaffRole): readonly Capability[] { return roleCapabilities[role]; }
export function roleAllowsCapability(role: StaffRole, capability: Capability): boolean { return roleCapabilities[role].includes(capability); }
