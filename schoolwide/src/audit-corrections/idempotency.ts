...
export async function startIdempotent<T>(
  ...
  const row = rows[0];
  if (!row) throw new AuditCorrectionError('IDEMPOTENCY_CONFLICT', 'Idempotency state is unavailable.', 409, true);
  ...
  if (row.operation !== input.operation || row.request_fingerprint !== input.fingerprint) {
    throw new AuditCorrectionError('IDEMPOTENCY_CONFLICT', 'Idempotency key was already used for a different request.', 409);
  }
  ...
  throw new AuditCorrectionError('IDEMPOTENCY_CONFLICT', 'The same correction is still being resolved.', 409, true);
}
