...
<style>
:root{font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color-scheme:light;background:#f4f6f8;color:#17202a}*{box-sizing:border-box}body{margin:0;background:#f4f6f8}button,input,select,textarea{font:inherit}button,input,select{min-height:44px}button:focus-visible,input:focus-visible,select:focus-visible,textarea:focus-visible{outline:3px solid currentColor;outline-offset:2px}.top{position:sticky;top:0;z-index:5;background:#17202a;color:#fff;padding:14px 20px;display:flex;align-items:center;justify-content:space-between;gap:16px}.brand{font-weight:800;letter-spacing:.02em}.status{font-size:.9rem}.wrap{max-width:1500px;margin:0 auto;padding:20px}.notice{padding:12px 14px;border:1px solid #aab4be;background:#fff;border-radius:10px;margin-bottom:16px}.metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:16px}.metric,.panel{background:#fff;border:1px solid #d6dde3;border-radius:12px}.metric{padding:16px}.metric strong{display:block;font-size:1.8rem}.panel{padding:16px;margin-bottom:16px}h1,h2{margin-top:0}.filters{display:grid;grid-template-columns:2fr repeat(3,minmax(150px,1fr)) auto;gap:10px;align-items:end}.field{display:flex;flex-direction:column;gap:5px}.field label{font-weight:650;font-size:.9rem}.field input,.field select,.field textarea{border:1px solid #9da9b3;border-radius:8px;padding:9px 10px;background:#fff}.actions{display:flex;gap:8px;flex-wrap:wrap}.btn{border:1px solid #59636d;background:#fff;border-radius:8px;padding:8px 12px;cursor:pointer}.btn.primary{background:#17202a;color:#fff;border-color:#17202a}.btn:disabled{opacity:.55;cursor:not-allowed}.table-wrap{overflow:auto}.movement{width:100%;border-collapse:collapse;min-width:960px}.movement th,.movement td{text-align:left;padding:10px;border-bottom:1px solid #e1e6ea;vertical-align:top}.movement th{font-size:.82rem;text-transform:uppercase;letter-spacing:.04em}.warning{font-weight:700}.warning[data-state="LATE"]::before{content:"Late — "}.warning[data-state="STALE"]::before{content:"Stale — "}.warning[data-state="UNCONFIGURED"]::before{content:"Threshold unavailable — "}.muted{color:#5e6872}.empty{padding:24px;text-align:center;color:#5e6872}.search-results{display:grid;gap:8px;margin-top:12px}.search-card{border:1px solid #d6dde3;border-radius:9px;padding:10px}.queue{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:10px}.queue-item{border:1px solid #d6dde3;border-radius:9px;padding:10px}dialog{border:0;border-radius:12px;max-width:520px;width:calc(100% - 32px);box-shadow:0 20px 60px rgba(0,0,0,.25)}dialog::backdrop{background:rgba(0,0,0,.45)}dialog form{display:grid;gap:12px}textarea{min-height:110px;resize:vertical}.dialog-actions{display:flex;justify-content:flex-end;gap:8px}.sr-status{min-height:1.4em;margin-top:8px}@media(max-width:900px){.metrics{grid-template-columns:repeat(2,1fr)}.filters{grid-template-columns:1fr 1fr}.filters .wide{grid-column:1/-1}}@media(max-width:520px){.wrap{padding:12px}.metrics{grid-template-columns:1fr 1fr}.filters{grid-template-columns:1fr}.filters .wide{grid-column:auto}.top{align-items:flex-start;flex-direction:column}}
</style>
</head>
<body>
...
(function(){
  ...
  function queryString(){var p=new URLSearchParams();if(els.destination.value)p.set('destinationId',els.destination.value);if(els.warning.value)p.set('warning',els.warning.value);var section=els.section.value.trim();if(section)p.set('sectionId',section);var s=p.toString();return s?'?'+s:'';}
  ...
  function renderMovement(board){
    els.out.textContent=String(board.summary.out);els.waiting.textContent=String(board.summary.waiting);els.late.textContent=String(board.summary.late);els.stale.textContent=String(board.summary.stale);renderDestinations(board.destinations||[]);
    safeRender(function(){
  ...
  async function refresh(){if(!token){setConnection();return;}try{var board=await api('/api/v1/security/live-passes'+queryString());lastBoard=board;pollAfterMs=board.transport&&board.transport.pollAfterMs||5000;renderMovement(board);setConnection();}catch(e){els.status.textContent=e.message;}finally{clearTimeout(pollTimer);if(token)pollTimer=setTimeout(refresh,pollAfterMs);}}
  ...
  els.refresh.addEventListener('click',refresh);els.destination.addEventListener('change',refresh);els.warning.addEventListener('change',refresh);els.section.addEventListener('change',refresh);els.search.addEventListener('input',function(){clearTimeout(els.search._timer);els.search._timer=setTimeout(search,250);});els.confirm.addEventListener('click',function(ev){ev.preventDefault();performAction();});els.dialog.addEventListener('close',function(){if(returnFocus&&document.contains(returnFocus))returnFocus.focus();returnFocus=null;});
