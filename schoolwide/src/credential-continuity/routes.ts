...
export function registerCredentialContinuityRoutes(app:FastifyInstance,options:{authentication:StaffAuthenticationService;authorization:StaffAuthorizationService;continuity:CredentialContinuityService}):void{
  app.post('/api/v1/internal/migration/credentials/continuity-plan',async(request,reply)=>{const id=requestId(request,reply);try{const principal=await options.authentication.authenticate(bearer(request));const schoolId=options.continuity.resolveSchoolId(principal,'admin.migration.credential_continuity');options.authorization.requireSchoolCapability(principal,schoolId,'admin.migration.credential_continuity');const parsed=body.safeParse(request.body);if(!parsed.success)return reply.code(400).send({code:'CREDENTIAL_CONTINUITY_INPUT_INVALID',message:'A certified shadow import run is required.',requestId:id,retryable:false});const result=await options.continuity.plan(principal,schoolId,parsed.data.shadowImportRunId);return{...result,schoolId,requestId:id};}catch(error){if(sendError(reply,error,id))return;throw error;}});
}
