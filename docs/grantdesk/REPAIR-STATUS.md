...
The release preserved schema `2026-09-05-session-a`, the five-file Apps Script project, stable `/exec`, existing deployment ID, school-domain access, existing workbook, late-sign-in review, teacher absences, streaks, and Hall Pass rules. The old 202-count card did not carry forward because the new contention counter covers Hall Pass operations only; Daily Check-In no longer retries that lock.

Release evidence: local 73 handoff mappings, structural suite, and 311 behavioral checks across 20 areas passed. New cases prove that a refused workbook lock still returns a recorded student state, an interrupted response can safely replay its consumed proof, a thirty-student burst remains visible to the teacher before Sheet flush, replayed inbox data cannot duplicate a row, and only the teacher or owned trigger can invoke the public flusher. The dependency-complete GitHub gate and browser checks passed, [PR #71](https://github.com/Grantauch/the-desk/pull/71) merged as `47ce8b0ae085528ba7c6cef36a1d2c4c7ccfaa42`, and [the in-place deployment workflow](https://github.com/Grantauch/the-desk/actions/runs/34375381483) released Version 23. The protected synthetic smoke returned `ok: true` with production facts unchanged; student, kiosk, check-in, and teacher modes initialized without errors. Real-class field evidence remains pending.
...
| 9 | OPEN | Shared verification is implemented; production execution remains unverified. Branch protection (11) is separate. |
| 10 | PUSHED | Netlify is configured to call the canonical npm run verify command, matching GitHub Actions and local publishing. Exact Netlify production execution remains unverified until dashboard access is available. |
...
| 12 | OPEN | Narrow authenticated editor writes remain intentional. Netlify verification backstop is pushed; production execution remains unverified. |
...
| 16 | VERIFIED | Resource sync uses the shared eligibility helper, preserves linked and null/empty/absent-href coming-soon resources and assignments, validates both outputs before writes, exports only public fields, and preserves private source files. |
| 17 | VERIFIED | Local editor listing and assignment validation use shared eligibility. Coming-soon status is visible; an actual HTTP save/publish fixture preserves placeholders and private-only assignment history while rejecting concurrent mutations. |
| 18 | VERIFIED | Ordinary local/editor publishing never runs resources:sync. The full gate and synthetic editor publication pass without private inventories; public catalog and private source bytes are preserved. |
...
| 58 | VERIFIED | Both legacy batch scripts delegate to publish.bat and the shared publisher. No automatic all-files staging or index-lock deletion. Intended batches, failure stops, source stability, upload retry and remote SHA confirmation are covered with synthetic repositories. |
...
Next executable task: continue independent website/content/accessibility repairs or repository protection (11). Exact Netlify production verification (10/39) needs authenticated dashboard access.

Problem 11 is now OPEN because administrator access is available; main currently has no branch protection. The START-HERE follow-up is complete. The curriculum publishing gaps are resolved in the current batch. Problem 23 still requires Grant's content mapping. Problem 39 is BLOCKED_EXTERNAL: no deployment fingerprint exposed by GitHub, and the currently available Netlify/GitHub browser session requires sign-in. No exact deployment is inferred. This does not block independent release work.
...
Implementation `adb14b472bf7d57eed1d1218558eda5e732acf5b` was pushed to main and the remote SHA matched. [Canonical GitHub Actions verification passed](https://github.com/Grantauch/the-desk/actions/runs/34054617391). The homepage, resources, editor, pass and check-in routes returned HTTP 200 after the push; availability does not prove the deployed source version. Netlify dashboard sign-in is required for the exact deployment fingerprint and production gate verification. Apps Script Version 18 and the private workbook are unchanged.

## September 7 website design edition
