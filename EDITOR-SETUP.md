...
### 3. Add the private Netlify publishing token

In **Project configuration → Environment variables**, add:

- `GITHUB_EDITOR_TOKEN` — the fine-grained token from GitHub

`EDITOR_EMAIL` is an optional configurable override. When Netlify does not expose it at runtime, the editor still verifies Grant's exact Identity account against a one-way fingerprint stored in the server code; the email address itself is not stored in the repository.

Optional overrides also exist for `EDITOR_REPOSITORY` and `EDITOR_BRANCH`; the defaults are `Grantauch/the-desk` and `main`.

Trigger one new production deploy after adding or replacing the token. Then sign in at `/editor/` and publish a harmless wording change as the final check.

## Security notes

- The GitHub key stays server-side inside Netlify Functions. It is never sent to the browser.
- Every read and write requires a valid Netlify Identity session for Grant's exact account. The optional `EDITOR_EMAIL` variable can override the built-in account fingerprint.
